...
FILESYSTEM_IMAGE=$1
FILESYSTEM_IMAGE_FMT=$(qemu-img info "$FILESYSTEM_IMAGE" | grep 'file format' | cut -d ' '  -f 3)
...
${QEMU} -nographic \
    ...
    -m 1G \
    -bios "$U_BOOT_BIN" \
    -drive if=none,id=disk0,file=$FILESYSTEM_IMAGE,format=$FILESYSTEM_IMAGE_FMT \
